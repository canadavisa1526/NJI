      ...
      >
          ...
          <div className="bg-white dark:bg-gray-800">
            {pricing.map((row, index) => (
              <motion.div
                ...
                className={`grid grid-cols-3 gap-4 p-4 border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors ${
                  index === pricing.length - 1 ? 'border-b-0' : ''
                }`}
              >
                <div className="flex items-center space-x-3">
                  <div className={`w-3 h-3 rounded-full bg-gradient-to-r ${row.color}`}></div>
                  <span className="font-medium text-gray-800 dark:text-gray-200">{row.ageGroup}</span>
                </div>
                <div className="flex items-center space-x-2">
                  <DollarSign className="w-4 h-4 text-[#FAA71A]" />
                  <span className="text-gray-600 dark:text-gray-400">{row.visitorInsurance}</span>
                </div>
                <div className="flex items-center space-x-2">
                  <DollarSign className="w-4 h-4 text-[#FAA71A]" />
                  <span className="text-gray-600 dark:text-gray-400">{row.superVisa}</span>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        >
          <p className="text-sm text-yellow-800 dark:text-yellow-200 flex items-start space-x-2">
            ...
            <span>
              <strong>Note:</strong> Rates vary by medical history, coverage amount, deductible, and company. 
